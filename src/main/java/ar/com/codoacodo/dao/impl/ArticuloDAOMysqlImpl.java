...
public class ArticuloDAOMysqlImpl implements IArticuloDAO {
	...
	public Articulo getById(Long id) throws Exception {
		...
		// 4 - extraer los datos
		/*
		 * if(resultSet.next()) { Long idDb = resultSet.getLong("id");//dame un Long en
		 * la posicion id y asignale una variable String titulo =
		 * resultSet.getString("titulo"); String autor = resultSet.getString("autor");
		 * Float precio = resultSet.getFloat("precio"); String img =
	...
	public Articulo fromResultsetToArticulo(ResultSet resultSet) throws Exception {// por parametros le pasamos un
																					// ResultSet
	...
	public List<Articulo> findAll() throws Exception {
		...
		// 4 - extraer los datos
		/*
		 * while(resultSet.next()) {//mientras alla resultado =>true|false Long idDb =
		 * resultSet.getLong("id");//dame un Long en la posicion id y asignale una
		 * variable String titulo = resultSet.getString("titulo"); String autor =
		 * resultSet.getString("autor"); Float precio = resultSet.getFloat("precio");
		 * String img = resultSet.getString("img");
		 * 
		 ...
		 * ArrayList aca dentro y eliminamos la linea de la instanciacion n�93
		 * 
		 */
	...
	public void create(Articulo articulo) throws Exception {
		...
		// 3 - armar el java.sql.Statement -- por ser un preparedStatement necesito
		// hacer la consulta primero
		...
		if (res.next()) {
			articulo.setId(res.getLong(1));/*
											 * seteamos al id del Articulo el Long que que me asigno
	...
	public void delete(Long id) throws Exception {
		...
		int resultSet = statement.executeUpdate(sql);/*
														 * en delete se utiliza executeUpdate que me devuelve un entero,
														 * 0 si no se puede y 1 si se puede eliminar el registro
														 */
